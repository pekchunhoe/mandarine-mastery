...
Limits: 500 characters per sentence, 2000 per paragraph, 6000 per essay, 8000 total context characters, 32 KiB JSON body, 16 vocabulary candidates, and 18,000 output characters plus bounded schema fields. Each action has a smaller output-token cap. Calls use at most one Gemini request with SDK retries disabled. Flash Lite uses a zero thinking budget; a `gemini-3.8-flash` override uses low thinking, while unknown overrides receive no optional thinking setting. The browser keeps successful, exact normalized requests in memory for five minutes, keyed by model, action, relevant context and candidate IDs; changed input misses the cache, and errors, cancellations, 429s, timeouts and malformed replies are never cached. Upstream timeout is 35 seconds and browser timeout is 40 seconds, leaving time for a controlled response before Vercel's 40-second function limit. Close, navigation and replacement requests cancel local processing, and stale results cannot replace current feedback. Provider processing already underway may still incur usage. Errors use friendly Mandarin. Server logs contain only action, model, thinking mode, duration and a sanitized outcome category. No AI result is saved to the draft.
