...
One primary endpoint, `POST /api/gemini`, accepts ten predefined actions: `sentence_hint`, `sentence_check`, `sentence_expand`, `sentence_vivid`, `vocabulary_help`, `essay_next_step`, `paragraph_expand`, `paragraph_vivid`, `paragraph_review`, `essay_review`. Public action constants/input rules live in `js/tutor-actions.js`; prompts and output schemas remain in `server/ai-contract.js`. Requests contain only `{action, activity, context}` with action-specific fields. The server revalidates independently and rejects unexpected output fields.

Hybrid vocabulary recommendations share one server resolver and card UI across Sentence Builder's free-writing and block-building modes and Guided Essay's current paragraph. Relevant curated candidates are preferred. Gemini may supplement them with Chinese words/phrases, pinyin, Chinese definitions, contextual examples and reasons, targeting 5–8 relevant terms without a fixed source ratio. Selected IDs are validated against server-resolved candidates; stored fields remain authoritative. Generated terms matching any published library word resolve to that stored record. Returned cards carry `source: "library"` or `source: "ai"`; generated cards need no ID and use the same arbitrary-text pronunciation controls. Exact expressions and known library synonyms are deduplicated, and the prompt also asks Gemini to avoid semantic duplicates. Invalid supplements are filtered independently; a partially filtered malformed response is displayed but not cached.

Guided Essay's expansion and vivid-writing tools use the existing active writing-point paragraph context, title and at most one immediately preceding paragraph, never the full essay. They offer concise suggestions, a reference version, an explanation and a copy button. Copying or dismissing feedback never edits the draft. Empty paragraphs are rejected locally; vivid-writing also requires at least six composition characters. Whole-essay review remains in the separate full-essay area.

The server selects models only after validating the action: `sentence_hint`, `sentence_check`, `sentence_expand`, `sentence_vivid`, `vocabulary_help`, `essay_next_step`, `paragraph_expand`, and `paragraph_vivid` use Flash-Lite; `paragraph_review` and `essay_review` use Flash. There is no student model selector or automatic model fallback. Limits are 500 characters per sentence, 2000 per paragraph, 6000 per essay, 8000 total context characters, 32 KiB JSON body, 16 vocabulary candidates, and 18,000 output characters plus bounded schema fields. Output caps are 320, 380, 500, 500, 1400, 480, 800, 800, 1200, and 1800 tokens in that action order. Only vocabulary's previous cap increased, to accommodate supplementary definitions and pinyin; the two new paragraph actions use 800 tokens for a concise reference and teaching notes without echoing the original paragraph. Calls use at most one Gemini request with SDK retries disabled. The Interactions request sends `max_output_tokens` and uses `thinking_level: 'minimal'` for advanced reviews; the fast route leaves reasoning at the model default. The browser keeps successful, exact normalized requests in memory for five minutes using a versioned activity/action/context key; changed input misses the cache, and errors, cancellations, 429s, timeouts and malformed replies are never cached. Upstream timeout is 35 seconds and browser timeout is 40 seconds. Close, navigation and replacement requests cancel local processing, and stale results cannot replace current feedback. Provider processing already underway may still incur usage. Errors use friendly Mandarin. Logs contain only action, route class, selected model, duration, sanitized outcome, and available upstream status/code. No AI result is saved to the draft.
