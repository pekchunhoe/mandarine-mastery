...
import { getActiveEssayContentsByEssayId, getEssayContentsByEssayId } from '../js/essay-content-service.js';
import { deriveEssayTraining, sentenceCue, splitEssayParagraphs, writingLengthGuidance } from '../js/essay-training-service.js';
import { guidedEssayWriting, modelEssayStudy } from '../activities/essayTraining.js';
import { deriveEssaySentences, resolveReferenceParagraph, splitChineseSentences } from '../js/essay-sentence-service.js';
import { state } from '../js/state.js';

test('Chinese sentence tokenizer retains punctuation, quotes, final fragments and paragraphs', () => {
  assert.deepEqual(splitChineseSentences('今天是星期天。我和家人去公园。天气很好！'), ['今天是星期天。', '我和家人去公园。', '天气很好！']);
  assert.deepEqual(splitChineseSentences('你今天开心吗？我很开心。'), ['你今天开心吗？', '我很开心。']);
  assert.deepEqual(splitChineseSentences('妈妈说：“快来吃饭吧！”我马上跑过去。'), ['妈妈说：“快来吃饭吧！”', '我马上跑过去。']);
  assert.deepEqual(splitChineseSentences('我想了很久……最后，我明白了。'), ['我想了很久……', '最后，我明白了。']);
...
test('reference examples resolve to the complete source paragraph across supported line breaks', () => {
  ...
  for (const separator of ['\n', '\n\n', '\r\n']) {
    ...
    assert.equal(resolveReferenceParagraph(essay, { paragraphIndex: 0, referenceSentence: '第二句。' }), first);
    assert.equal(resolveReferenceParagraph(essay, { paragraphIndex: 1, referenceSentence: '第二段第一句。' }), second);
  }
});

test('reference examples select each matching paragraph without hardcoded content', () => {
  const essay = '开头第一句。开头第二句。\n\n经过第一句。经过第二句。\n\n结尾第一句。结尾第二句。';
  assert.equal(resolveReferenceParagraph(essay, { paragraphIndex: 0, referenceSentence: '开头第二句。' }), '开头第一句。开头第二句。');
  assert.equal(resolveReferenceParagraph(essay, { paragraphIndex: 1, referenceSentence: '经过第一句。' }), '经过第一句。经过第二句。');
  assert.equal(resolveReferenceParagraph(essay, { paragraphIndex: 2, referenceSentence: '结尾第二句。' }), '结尾第一句。结尾第二句。');
  assert.equal(resolveReferenceParagraph(essay, { referenceSentence: '经过第二句。' }), '经过第一句。经过第二句。');
  assert.equal(resolveReferenceParagraph(essay, { paragraphIndex: 9, referenceSentence: '不存在。' }), '不存在。');
});
...
test('training transformation preserves source text and creates variable-length paragraph scaffolds', () => {
  ...
  assert.ok(training.paragraphs.every((paragraph) => paragraph.keyPoints.length && paragraph.sentenceCue));
  assert.ok(training.paragraphs.every((paragraph) => paragraph.keyPoints.every((point) => point.length <= paragraph.text.length)));
...
test('essay content service supports multiple versions without exposing inactive content', () => {
  ...
  assert.ok(getEssayContentsByEssayId(content.essayId).length >= getActiveEssayContentsByEssayId(content.essayId).length);
});

test('model study renders the selected model while guided writing keeps it hidden until reveal', () => {
  const previousStorage = globalThis.localStorage;
  globalThis.localStorage = { setItem() {} };
  const topic = titles.find((item) => item.gradeMin === 3), content = contents.find((item) => item.essayId === topic.id);
  const fakeRoot = () => ({
    innerHTML: '', listeners: {},
  ...
  try {
    ...
    assert.ok(splitEssayParagraphs(content.content).every((paragraph) => splitChineseSentences(paragraph).every((sentence) => studyRoot.innerHTML.includes(sentence))));
    ...
    guidedRoot.listeners.click({ target: { closest: () => ({ id: 'guided-reveal', dataset: {} }) } });
    assert.ok(splitEssayParagraphs(content.content).every((paragraph) => splitChineseSentences(paragraph).every((sentence) => guidedRoot.innerHTML.includes(sentence))));
  } finally {
...
test('guided final hint renders its complete paragraph and retains the current student draft', () => {
  ...
  state.drafts = {
    [key]: { text: '我的草稿。', lines: ['我的草稿。'], checklist: {}, helpLevel: 1, reveal: false },
  };
  const root = {
    innerHTML: '', listeners: {},
    addEventListener(type, listener) { this.listeners[type] = listener; },
  ...
  try {
    ...
    for (let level = 1; level < 5; level += 1)
      root.listeners.click({ target: { closest: () => ({ id: 'guided-more-help', dataset: {} }) } });
